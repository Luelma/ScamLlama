...
class ChecklistScorer {
    ...
    func calculateScore(checkedIDs: Set<String>) -> (score: Double, riskLevel: RiskLevel) {
        ...
        // 5. Financial combo: financial flags + pressure tactics or info harvesting = extra danger
        let hasFinancial = checkedItems.contains { $0.category == "Financial Red Flags" }
        let hasPressure = checkedItems.contains { $0.category == "Pressure Tactics" }
        let hasInfoHarvesting = checkedItems.contains { $0.category == "Information Harvesting" }
        let comboBonus: Double = hasFinancial && (hasPressure || hasInfoHarvesting) ? 0.15 : 0.0
