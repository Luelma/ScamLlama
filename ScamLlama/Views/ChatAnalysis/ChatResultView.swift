            ...
            VStack(spacing: 8) {
                if let duration = context.contactDuration {
                    situationRow(
                        ...
                        risk: duration == .lessThanWeek ? .medium : .low
                    )
                }
                if let contactedFirst = context.contactedYouFirst {
                    situationRow(
                        label: "Unsolicited Contact",
                        value: contactedFirst ? "Yes" : "No",
                        risk: contactedFirst ? .high : .low
                    )
                }
                ...
                if let payments = context.hasMadePayments {
                    situationRow(
                        label: "Payments Made",
                        value: payments ? "Yes" : "No",
                        risk: payments ? .high : .low
                    )
                }
                if let sharedInfo = context.hasSharedPersonalInfo {
                    situationRow(
                        label: "Shared Personal Info",
                        value: sharedInfo ? "Yes" : "No",
                        risk: sharedInfo ? .high : .low
                    )
                }
                if let clickedLinks = context.hasClickedLinks {
                    situationRow(
                        label: "Clicked Links",
                        value: clickedLinks ? "Yes" : "No",
                        risk: clickedLinks ? .medium : .low
                    )
                }
            }
        }
...
struct NextMovePredictionCardView: View {
    ...
    var body: some View {
        ...
        .frame(maxWidth: .infinity, alignment: .leading)
        .padding()
        .background(Color(red: 0.12, green: 0.22, blue: 0.42).opacity(0.08))
        .clipShape(RoundedRectangle(cornerRadius: 16))
        .padding(.horizontal)
    }
}
