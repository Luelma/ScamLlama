            ...
            if showContextSection {
                VStack(spacing: 16) {
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        Text("How long have you been in contact?")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                        Picker("Duration", selection: $viewModel.contactDuration) {
                            Text("Not set").tag(ContactDuration?.none)
                            ForEach(ContactDuration.allCases, id: \.self) { duration in
                                Text(duration.rawValue).tag(ContactDuration?.some(duration))
                            }
                        }
                        .pickerStyle(.segmented)
                    }
                    ...
                    // Toggles
                    Toggle("Did they contact you first?", isOn: $viewModel.contactedYouFirst)
                        .font(.subheadline)
                    Toggle("Have they asked for money?", isOn: $viewModel.hasBeenAskedForMoney)
                        .font(.subheadline)
                    Toggle("Have you already made payments?", isOn: $viewModel.hasMadePayments)
                        .font(.subheadline)
                    Toggle("Have you shared personal info (SSN, bank, etc.)?", isOn: $viewModel.hasSharedPersonalInfo)
                        .font(.subheadline)
                    Toggle("Have you clicked any links they sent?", isOn: $viewModel.hasClickedLinks)
                        .font(.subheadline)
                }
                .padding()
                .background(Color(.secondarySystemBackground))
